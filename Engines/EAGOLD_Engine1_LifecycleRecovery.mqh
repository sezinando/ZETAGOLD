...
// Recovery lot source: only the initial lot or the latest activated Recovery
// position is allowed to drive the R11 progression. Other market orders in
// the same direction (R7/R9/R4) must never contaminate the Recovery sequence.
bool GetLatestRecoveryLot(int direction,double &latestLot,int &latestTicket){int type=(direction==OP_BUY?OP_BUY:OP_SELL);string tag=(direction==OP_BUY?"EAGOLD BUY RECOVERY":"EAGOLD SELL RECOVERY");latestLot=NormalizeLot(Lot);latestTicket=-1;datetime latestTime=0;bool found=false;for(int i=OrdersTotal()-1;i>=0;i--){if(!OrderSelect(i,SELECT_BY_POS,MODE_TRADES))continue;if(!IsEAGOLDOrder()||OrderType()!=type)continue;if(StringFind(OrderComment(),tag,0)<0)continue;datetime t=OrderOpenTime();int ticket=OrderTicket();if(!found||t>latestTime||(t==latestTime&&ticket>latestTicket)){found=true;latestTime=t;latestLot=OrderLots();latestTicket=ticket;}}return(found);}
