...
#include "../Core/EAGOLD_R10_V2_IntegrationTest.mqh"
...
#include "../Core/EAGOLD_LotProgressionAudit.mqh"
bool IsR13Order();
...
#include "../Core/EAGOLD_DualCycleManager.mqh"
...
int OnInit()
{
if(EnableModularizationDebug)EAGOLD_R10V2RunEnablementTestMatrix();if(EnableModularizationDebug)EAGOLD_R10V2RunIntegrationTest();if(EnableModularizationDebug)EAGOLD_R10V2RegressionSuiteRun();if(EnableModularizationDebug)EAGOLD_R10V2RunFailureRegression();if(EnableModularizationDebug)EAGOLD_RunLotProgressionAudit();ObjectsDeleteAll(0,ENGINE_MARKER_PREFIX);ObjectsDeleteAll(0,R10_MARKER_PREFIX);ArrayResize(g_r9ProcessedTickets,0);g_r9HedgeActive=false;g_r10LastAction=0;g_r10RecoveryCycleActive=false;g_r10RecoveryStartEquity=0.0;g_r10RecoveryWorstEquity=0.0;g_eagoldTickPolicy=EAGOLD_TICK_CONTINUE;if(!EAGOLDValidateOwnershipConfiguration()||!IsR13OwnershipConfigurationValid()){Print(EA_NAME," INIT BLOCKED: unsafe order ownership configuration. EAGOLD will not trade this chart.");return(INIT_FAILED);}ZG_IntelligenceReset(g_zgIntelligenceDecision);ZG_IntelligenceEvaluate(g_zgIntelligenceDecision);EAGOLD_DualCycleReset();Print(EA_NAME," TRADE DIRECTION: ",ZG_ManualDirectionName()," operational=",ZG_OperationalDirectionName()," | INTELLIGENCE=",ZG_DirectionName(g_zgIntelligenceDecision.Direction));Print(EA_NAME," INTELLIGENCE MODE: ", (ZG_DirectionFilterEnabled()?"DIRECTION FILTER ON":"SHADOW/OFF")," dir=",g_zgIntelligenceDecision.Direction," edge=",DoubleToString(g_zgIntelligenceDecision.Edge,2)," confidence=",DoubleToString(g_zgIntelligenceDecision.Confidence,2));EAGOLD_R12Reset(g_r12State);EAGOLD_R12ResetHistory();EAGOLD_ExcursionTrackerReset();EAGOLD_AdaptiveProfitGuardReset();EAGOLD_R10V2ScenarioHarnessRun();EAGOLD_R10V2CalibrationRun();EAGOLD_R10V2ContextReset(g_r10V2Context);g_r10V2Opportunity=EAGOLD_R10V2_OPP_NONE;EAGOLD_R10V2DecisionContractReset(g_r10V2DecisionContract);g_r10V2Target.found=false;g_r10V2Target.ticket=-1;g_r10V2CandidateLots=0.0;g_r10V2DesiredLots=0.0;g_r10V2CapitalCapacity=0.0;g_r10V2ExposureCapacity=0.0;g_r10V2R11Capacity=0.0;g_r10V2BrokerCapacity=0.0;g_r10V2AuthorizedLots=0.0;EAGOLD_R10V2R11ReductionCapacityReset(g_r10V2R11ReductionCapacity);EAGOLD_R10V2StructuralProjectionReset(g_r10V2StructuralProjection);EAGOLD_R10V2BalancedPairReset(g_r10V2BalancedPair);EAGOLD_R10V2BalancedProjectionReset(g_r10V2BalancedProjection);g_r10V2PreExecutionGate=false;g_r10V2PreLiveGate=false;g_r10V2PreLiveReason=EAGOLD_R10V2_PRELIVE_FEATURE_DISABLED;g_r10V2PreExecutionReason=EAGOLD_R10V2_VALIDATION_NO_OPPORTUNITY;EAGOLD_R10V2CapitalLedgerReset();EAGOLD_R10V2ComparativeReset();if(EAGOLD_PersistenceEnabled())LoadPersistedStrategicState();else{g_modPanelMinProfit=0.0;g_modPanelMaxLots=0.0;g_modPanelInitialized=true;g_panelMinProfit=0.0;g_panelMaxLots=0.0;g_panelInitialized=true;}g_r1LastDecision="DISABLED";g_r1LastReason="";g_r1LastDecisionTime=0;R13ResetObserverState(g_r13Observer);R9SeedExistingPositions();R10RecoveryUpdateState();EAGOLD_R12Update(g_r12State);R13Observe(g_r13Observer);EAGOLD_ModPanelUpdate();EAGOLD_RealizationCascadeUpdate();EAGOLD_ChartBasketGuidesUpdate();PersistAllState(true);EAGOLD_CreateFirstOrdersAtomic();EAGOLD_ExcursionTrackerObserve();EAGOLD_AdaptiveProfitGuardObserve();R10RecoveryUpdateState();R13Observe(g_r13Observer);EAGOLD_ModPanelUpdate();EAGOLD_RealizationCascadeUpdate();EAGOLD_ChartBasketGuidesUpdate();PersistAllState(true);return(INIT_SUCCEEDED);}
...
if(EAGOLD_EconomicExecutionAllowed()){EAGOLD_ActionResult r1Result=EAGOLD_CreateFirstOrdersAtomic();if(r1Result!=EAGOLD_ACTION_BLOCKED){int r1TelemetryDirection=HeavyDirection();if(ZG_DirectionFilterEnabled()){if(g_zgIntelligenceDecision.Direction==ZG_DIR_BUY)r1TelemetryDirection=OP_BUY;else if(g_zgIntelligenceDecision.Direction==ZG_DIR_SELL)r1TelemetryDirection=OP_SELL;}EAGOLD_ApplyActionResult(r1Result,"R1","FIRST_SEED",r1TelemetryDirection,Lot);}}if(EAGOLD_EconomicExecutionAllowed())EAGOLD_DualCycleObserve();if(EAGOLD_EconomicExecutionAllowed()){EAGOLD_ActionResult r9Result=Rule9DetectActivatedOrdersTransactional();if(r9Result!=EAGOLD_ACTION_BLOCKED)EAGOLD_ApplyActionResult(r9Result,"R9","HEDGE",HeavyDirection(),0.0);}if(EAGOLD_EconomicExecutionAllowed()){EAGOLD_ActionResult r7Result=EAGOLD_R7EnsureMissingDirectionTransactional();if(r7Result!=EAGOLD_ACTION_BLOCKED)EAGOLD_ApplyActionResult(r7Result,"R7","KEEP_ALIVE",HeavyDirection(),Lot);}if(EAGOLD_EconomicExecutionAllowed())BuyMachine();if(EAGOLD_EconomicExecutionAllowed())SellMachine();if(EAGOLD_EconomicExecutionAllowed())TrailAllStopOrders();if(EAGOLD_EconomicExecutionAllowed())R13Observe(g_r13Observer);EAGOLD_ExcursionTrackerObserve();ObjectsDeleteAll(0,R10_MARKER_PREFIX);EAGOLD_ModPanelUpdate();EAGOLD_RealizationCascadeUpdate();EAGOLD_ChartBasketGuidesUpdate();PersistAllState(false);}
